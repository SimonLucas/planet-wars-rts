...
class DQN_Defense_Multi_Simulation_Agent(PlanetWarsPlayer):
    def __init__(self, model_path=None):
        ...
        if os.path.exists(self.model_path):
            try:
                ...
                print(f"✅ DQN_Defense_Multi_Simulation_Agent successfully loaded weights from {self.model_path}")
            except Exception as e:
                print(f"⚠️ Warning: Failed loading state_dict in DQN_Defense_Multi_Simulation_Agent ({e})")
        else:
            print(f"📡 DQN_Defense_Multi_Simulation_Agent checkpoint not found at {self.model_path}. Running default random layout.")
    ...
    def prepare_to_play_as(self, player: Player, params, *args, **kwargs) -> None:
        ...
        if self.params:
            self.transporter_speed = getattr(self.params, 'transporter_speed', 2.0)
            self.map_width = getattr(self.params, 'width', 25.0)
            self.map_height = getattr(self.params, 'height', 25.0)

        for opp in self.sim_opponents.values():
            opp.prepare_to_play_as(self.player.opponent(), self.params)

        for arg in list(args) + list(kwargs.values()):
            if hasattr(arg, 'planets') and not self._matrix_initialized:
                self._ensure_matrix_initialized(arg)
                break

    def _ensure_matrix_initialized(self, game_state: GameState):
        # Fixed Issue 4: Force initialization if game_tick is 0 to wipe caches across layout generation transitions
        if getattr(game_state, 'game_tick', 0) == 0:
            self._matrix_initialized = False

        if self._matrix_initialized and self.eta_matrix is not None:
            return
        
        total_planets = len(game_state.planets)
        self.eta_matrix = np.zeros((total_planets, total_planets), dtype=np.float32)
        
        speed = getattr(self.params, 'transporter_speed', self.transporter_speed)
        if speed <= 0.01:
            speed = 2.0

        for i in range(total_planets):
            p_src = game_state.planets[i]
            for j in range(total_planets):
                if i == j:
                    self.eta_matrix[i][j] = 0.0
                else:
                    p_dest = game_state.planets[j]
                    dist = p_src.position.distance(p_dest.position)
                    self.eta_matrix[i][j] = float(dist / speed)
    ...
    def _is_attack_viable(self, src_id, dest_id, src_ships, dest_ships, dest_growth):
        if self._matrix_initialized and src_id < self.eta_matrix.shape[0] and dest_id < self.eta_matrix.shape[1]:
            eta = self.eta_matrix[src_id][dest_id]
        else:
            eta = 1.0
        estimated_defense = dest_ships + dest_growth * eta
        return src_ships > estimated_defense

    def _compute_pressures_and_etas(self, game_state: GameState):
        self._ensure_matrix_initialized(game_state)

        current_tick = getattr(game_state, 'game_tick', 0)
        total_planets = len(game_state.planets)
        incoming_friendly_pressure = {i: 0.0 for i in range(total_planets)}
        incoming_enemy_pressure = {i: 0.0 for i in range(total_planets)}
        min_enemy_eta = {i: float('inf') for i in range(total_planets)}

        current_speed = getattr(self.params, 'transporter_speed', self.transporter_speed)
        ...
        for p_src in game_state.planets:
            ...
            if transporter is not None:
                ...
                if dest_id is not None and dest_id < total_planets:
                    ...
                    if pos is not None and vel is not None:
                        ...
                        src_id = getattr(p_src, 'id', None)
                        if src_id is not None and src_id < self.eta_matrix.shape[0]:
                            eta = self.eta_matrix[src_id][dest_id]
                        else:
    ...
    def _state_to_tensor(self, game_state: GameState, precomputed_pressures=None) -> torch.Tensor:
        ...
        for i in range(self.num_planets):
            if i < len(game_state.planets):
                ...
                state_list.extend([owner_val, n_ships_val / 100.0, p.growth_rate, p.position.x / self.map_width, p.position.y / self.map_height, friendly_p.get(i, 0.0), enemy_p.get(i, 0.0)])
            else:
    ...
    def _heuristic_defense_fallback(self, game_state: GameState, enemy_p, min_enemy_eta) -> Action:
        ...
        for i, p in enumerate(game_state.planets):
            if p.owner == self.player and i != target_id:
                ...
                if self._matrix_initialized and i < self.eta_matrix.shape[0] and target_id < self.eta_matrix.shape[1]:
                    my_eta = self.eta_matrix[i][target_id]
                else:
                    dist = p.position.distance(target_planet.position)
                    my_eta = dist / current_speed
                if my_eta < enemy_eta:
    ...
    def _titans_offense(self, game_state: GameState) -> Action:
        ...
        for src_id, src_planet in enumerate(game_state.planets):
            ...
            for dest_id, dest_planet in enumerate(game_state.planets):
                ...
                if not self._is_attack_viable(src_id, dest_id, src_ships, dest_ships, dest_planet.growth_rate):
                    continue
        ...
        for candidate_action in candidates:
            ...
            for profile_key, baseline_bot in self.sim_opponents.items():
                cloned_state = game_state.model_copy(deep=True)
                fm = ForwardModel(cloned_state, self.params)
                
                fm.step({self.player: candidate_action, self.player.opponent(): baseline_bot.get_action(fm.state)})
                
                # Fixed Issue 1: Keep evaluating the state space instead of freezing into do_nothing
                for _ in range(self.horizon_ticks - 1):
                    if fm.is_terminal():
                        break
                    fm.step({self.player: self._get_simulation_action(fm.state), self.player.opponent(): baseline_bot.get_action(fm.state)})
                ...
                scores_dict[profile_key] = score
                
            action_scores.append((candidate_action, scores_dict))

        # Fixed Issue 3: Deleted the complex joint-dominance check loop. Directly optimize on portfolio averages.
        best_avg_score = -float('inf')
        best_fallback_action = Action.do_nothing()

        for action, scores_dict in action_scores:
    ...
    def get_agent_type(self) -> str:
        return "DQN_Defense_Multi_Simulation_Agent (DQN Defense / Cached Multi-Baseline Offense)"