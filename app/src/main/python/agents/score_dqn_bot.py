...
class score_dqn_bot(PlanetWarsPlayer):
    def __init__(self, model_path=None):
        ...
        if os.path.exists(self.model_path):
            try:
                ...
                print(f"✅ score_dqn_bot successfully loaded weights from {self.model_path}")
            except Exception as e:
                print(f"⚠️ Warning: Failed loading state_dict in score_dqn_bot ({e})")
        else:
            print(f"📡 score_dqn_bot checkpoint not found at {self.model_path}. Running default random weights.")
    ...
    def get_action(self, game_state: GameState) -> Action:
        ...
        # Instantiate GreedyHeuristicAgent to simulate a reactive opponent profile inside horizons
        opp_agent = GreedyHeuristicAgent()
        opp_agent.prepare_to_play_as(self.player.opponent(), self.params)
        ...
        for src_id, src_planet in enumerate(game_state.planets):
            ...
            for dest_id, dest_planet in enumerate(game_state.planets):
                ...
                # Rule check for reinforcements: only send if own target is under active pressure
        ...
        for candidate_action in candidates:
            ...
            # Step applying our candidate move along with a simulated live opponent action
            fm.step({self.player: candidate_action, self.player.opponent(): opp_agent.get_action(fm.state)})
            
            for _ in range(self.horizon_ticks - 1):
                if fm.is_terminal():
                    break
                fm.step({self.player: Action.do_nothing(), self.player.opponent(): opp_agent.get_action(fm.state)})
            ...
            if fm.is_terminal():
                ...
                # Extract learned value head prediction using the fully uncertainty-aware state mapping matches training context
    ...
    def get_agent_type(self) -> str:
        return "score_dqn_bot (DQN-Blended Evaluation / Uncertainty-Aware)"