# ==============================================================================
# FILE: agents/dqn_defense_agent_intuition_variant.py
# ==============================================================================
import os
import sys
import math
...
class DQN_Defense_Agent_Intuition_Variant(PlanetWarsPlayer):
    def __init__(self, model_path=None):
        ...
        if os.path.exists(self.model_path):
            try:
                ...
                print(f"✅ DQN_Defense_Agent_Intuition_Variant successfully loaded weights from {self.model_path}")
            except Exception as e:
                print(f"⚠️ Warning: Failed loading state_dict in DQN_Defense_Agent_Intuition_Variant ({e})")
        else:
            print(f"📡 DQN_Defense_Agent_Intuition_Variant checkpoint not found at {self.model_path}. Running default random layout.")

        # Cache the simulation opponent instance to avoid loading weights every game tick
        self.opp_agent = dqn_intuition_agent()
    ...
    def _titans_offense(self, game_state: GameState) -> Action:
        ...
        for candidate_action in candidates:
            ...
            for _ in range(self.horizon_ticks - 1):
                if fm.is_terminal():
                    break
                fm.step({self.player: Action.do_nothing(), self.player.opponent(): self.opp_agent.get_action(fm.state)})
    ...
    def get_agent_type(self) -> str:
        return "DQN_Defense_Agent_Intuition_Variant (DQN Defense / dqn_intuition Sim Offense)"